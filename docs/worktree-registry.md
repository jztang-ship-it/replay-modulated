...
- **Unique commits vs main:** 2 — `9454354c` (Track 1 + 2 build) + `fb2cfa7d` (§1.4 missing-half fix). NOT pushed, NOT merged. **Tip = `fb2cfa7d`.**
- **Uncommitted state:** `M package-lock.json` (npm-install drift, NOT committed); untracked throwaway recon scripts under `scripts/_recon-*.mjs`.
...
- **Self-measure — CORRECTED 2026-06-28 (SCOPED to each surface's own board-inner):** the 2026-06-27 numbers (loss 547/550, reg 518/521, anon 573/576) were UNRELIABLE — they read the reveal screen's `[data-h2h-board-inner]` (the FIRST of TWO on the boss/reveal mock), not the result overlay's. Real scoped heights after `fb2cfa7d` (§1.4 both-levers fix), vs ~650: intro/active 625/628 (play hero unchanged at 331/334), loss 503/504, social/registered-win 613/592, anon-win 668/647 (@430 fits; @390 +18 — the CTA tower, addressed by the parked winscreen pass). Result hero band dropped 331→~228–249. basketball build green; 230/230 H2H+Boss tests green (at `9454354c`).
- **NOT done (HOLD gate):** John's device glass on the real cold link (today's live boss UUID via hub resolver), both widths, all 5 states incl. incognito/`?claim=force` for the anon-win merged card. Full `npm test` + tri-sport build are the MERGE gate (not run yet — basketball-only for hold). **Anon-win @390 still ~18 over the Playwright-optimistic budget → the winscreen CTA-hierarchy pass (`feat/boss-winscreen-cta @ 33dfbd2e`, parked) is the intended follow-up; not yet rebased onto this.**
- **Last reviewed:** 2026-06-28 (§1.4 re-corrected + scoped re-measure; held for glass)
