...
~14 fields. Wider than `LandingAdapter` (10 fields) but the same structural pattern: data + components + lifecycle hooks.
...
| 01 | `phase-2/01-extract-pure-helpers` | Move `RosterGridScaleFit`, `RollingNumber`, `tierFromSalary`, `toRevealableCards`, `sleep`, etc. into `shared/views/_gameViewHelpers.ts`. Both sports import from shared. No behavior change. | ~250 | Low |
| 02 | `phase-2/02-define-adapter-+-skeleton` | Add `shared/views/GameAdapter.ts` (interface + types only). Add `shared/views/GameView.tsx` as a stub. Wires the contract without using it yet. | ~300 | Low |
| 03 | `phase-2/03-lift-state-+-fixed-helpers` | Move shared state hooks (`gameState`, `streak`, `balance`, `roster`, `winTier`, etc.) and leaderboard/log helpers (`submitToLeaderboard`, `checkLeaderboardRank`, `logHandToDb`) into shared core. Sport wrappers call into them. | ~600 | Medium — touches localStorage + leaderboard wiring just stabilized in PR #11 |
...
Per sub-PR: each preview gets a manual smoke test on both basketball and baseball, covering FTUE + post-FTUE, hold/draw/redraw, full reveal cycle, post-hand commentary, and leaderboard submission. Same checklist for every preview.

Final merge to main: same smoke check, then verify replayifs.com on both sports immediately after deploy.
