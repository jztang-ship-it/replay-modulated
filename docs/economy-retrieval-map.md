...
- **Pause points (the lines that stop the rake):**
  1. `BonusPoolPill` render gated at its call site in `shared/views/GameView.tsx`
     (`{economyEnabled && (<BonusPoolPill economyEnabled={economyEnabled} … />)}`
     in the non-challenge footer branch) — hides the surface.
  2. The 5% rake effect inside `BonusPoolPill` early-returns on `economyEnabled`
     (`if (!economyEnabled) return;`) — **decouples the rake from the render**, so
     it stays off even if the Pill render returns (it no longer silently
     reactivates). Scoped to the rake effect only; the pool-poll/display effects
     are untouched.
- **Legend money framing (sweep 4a):** the "Payout = bet × multiplier" caption
  clause and the entire STREAK WINS / `bonusRows` block (incl. the rake note) are
  gated on `economyEnabled` in the shared `LegendModal`. The tier threshold ladder
  (identity content) stays. Adapter `bonusRows` data untouched.
- **Inbox bonus-pool message (sweep 4):** `addBonusPoolMessage` (`shared/inbox/inbox.ts`)
  writes a "+N coins from the bonus pool" inbox row. It has **no call site** (dormant,
  post-beta). Guarded with a **required `economyEnabled` param + `if (!economyEnabled)
  return`** so it can't fire while the economy is gated — re-wiring must pass the flag
  deliberately (the util has no adapter context). Function kept (hide-don't-delete).
  - **Data note (NOT code):** existing persisted `inbox_messages` rows with
    `message_type='bonus_pool'` (seen in glass) are a DB artifact, not generated by
    any live path. **Deferred to the merge-gate one-time data wipe** (the same wipe
    that clears the dead `replaymod_balance` 4720): `DELETE FROM inbox_messages WHERE
    message_type = 'bonus_pool'`. Runs once, at the gate, against the confirmed DB —
    not on this branch.
