...
export function AthleteCard(props: Props) {
  ...
  const {
    ...
    performanceTag, pulse, shakeType, cardShakeType,
  } = props;

  const id = String((card as any).cardId ?? "");

  useEffect(() => {
    console.log(`[overlay] id=${id} flipped=${flipped} cardShakeType=${cardShakeType} stamp=${overlay.stamp}`);
  }, [flipped, cardShakeType]);
  ...
  const stableCard = useMemo(() => {
    ...
    if (!snap) return card;
    return { ...(card as any), tier:snap.tier, salary:snap.salary, projectedFp:snap.projectedFp } as PlayerCard;
  }, [card, id]);
  ...
  useEffect(() => {
    if (!flipped) {
      ...
      // Only clear latch if cardShakeType is also null — means new hand, not mid-reveal cycling
...
{/* Stamp outside preserve-3d context — renders on top */}
{overlay.stamp && flipped && (
  <div className={overlay.stamping ? "card-stamp" : "card-stamp-persist"}>
    <div style={stampStyle}>{overlay.stamp}</div>
  </div>
)}
</div>
);
