...
vi.mock('../hand/_lib/catalog.js',()=>({SPORTS:['basketball'],deal:m.deal,draw:m.draw,resolve:m.resolve,outcome:m.outcome}));
...
describe('authoritative hand boundary',()=>{
 ...
 it('fails closed when quota storage fails',async()=>{m.quota.mockRejectedValue(new Error('offline'));expect((await call({action:'start',request_id:id,sport:'basketball',season:'2425'})).status).toBe(503);expect(m.deal).not.toHaveBeenCalled();});
