...
it('old hands never enter todays daily board',async()=>{m.row.created_at='2020-01-01T00:00:00Z';expect((await call(submit)).status).toBe(200);expect(m.zadd).toHaveBeenCalledExactlyOnceWith('replay-free-play:v1:lb:v2:baseball:hand_best:alltime',{member:'user-1:hand-1',score:30});expect(m.expire).not.toHaveBeenCalled();});
