...
1. **Reuse working scaffolds before deriving new ones.** When an equivalent layout (sized card in a sized cell, scaled card in a strip, overlay positioned over a fixed canvas, etc.) is already working elsewhere in the codebase, COPY that scaffold. Do not hand-write a parallel one — divergences in `transformOrigin`, `display: flex` vs `position: absolute`, or natural-width constants will not be caught by JSDOM tests. At component-shell scale, this rule produced the `H2HBoardShell` extraction (shared/components/H2HBoardShell.tsx) — use that as the template when a similar reuse-vs-rebuild call is on the table: capture a pre-extraction baseline via the visual harness, extract the chrome wholesale (no semantic refactor), then verify the donor surface is byte-identical before any new consumer is wired.
