...
function getSalaryCap(config: SportConfig): { min: number; max: number } {
  if (typeof config.salaryCap === 'number') {
    return { min: 0, max: config.salaryCap };
  }
...
export class LineupGenerationEngine {
  static generateDeterministicLineup(
    ...
    const salaryCapLimits = getSalaryCap(config);
    const salaryCap = salaryCapLimits.max;
