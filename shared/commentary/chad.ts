...
// Basketball's controlled beta is free-to-play. Keep the normal commentary
// banks intact for a future economy-enabled product, but never surface their
// references to coins, bets, pools, or payouts in this mode.
...
export function chadMessage(topic: ChadTopic, freePlay = false): string {
  return pick((freePlay ? FREE_PLAY_BANKS : BANKS)[topic]);
}

/** Get all messages for a topic (for testing/preview). */
export function chadBank(topic: ChadTopic, freePlay = false): string[] {
  return [...(freePlay ? FREE_PLAY_BANKS : BANKS)[topic]];
}
