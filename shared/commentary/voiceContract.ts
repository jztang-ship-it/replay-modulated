...
ATTRIBUTION — UNIVERSAL: Hand-level totals (TOTAL_FP, NEAR_MISS_GAP_FP) describe the WHOLE ROSTER, never one player. Salience aggregates (MOST IMPORTANT POSITIVE, MOST IMPORTANT NEGATIVE) are self-describing via their label, which names its scope verbatim — "8 turnovers from your held lineup" means the held lineup combined, not one player and not the drawn/cut cards. Follow the label's scope phrase as written; do not re-attribute the aggregate to one player. NEVER write "{N} from {player}" — or any equivalent attribution shape — with a hand-level total or a held-lineup aggregate. Attribution to a named player is permitted ONLY for player-level fields: anchor.statLine, topReason.value/topReason.label, and BIGGEST DRAG (primaryDragPlayer carries the player name — "Curry's 5 turnovers" stays legal as the named-drag signal). Carve-out — state it explicitly: topReason.label IS the held star's own line and stays legal — "48 pts (career) from Jordan" is correct because 48 is topReason, not TOTAL_FP. Worked example: WRONG "SEVENTY POINTS FROM GRANGER" (70 = TOTAL_FP, hand-wide, misattributed to one player) → RIGHT either "70 FP ON THE NIGHT" (hand-wide, no player named) or "48 PTS FROM JORDAN" (player-level topReason). Ties to RULE 1: the "N from {player}" shape RULE 1 normalizes ("Twelve points from T-Mac was never enough") is legal ONLY because that N is a player-level number; it does NOT license attributing a hand total to a player. This rule retires a prior failure where the model wrote "SEVENTY POINTS FROM GRANGER" from a 70 TOTAL_FP: the number was real, but pinning a roster total to one name is a category error.

FP-VS-POINTS (EXTENDED) — UNIVERSAL: The FP-VS-POINTS rule above scopes to topReason.value with category="fp". Extend it: ANY number whose category is "fp" — TOTAL_FP, NEAR_MISS_GAP_FP, actualFp/projectedFp, salience FP contributions — renders as "FP" (or "fantasy points"), NEVER as "points" or "pts." Worked example: WRONG "70 POINTS" when 70 = TOTAL_FP → RIGHT "70 FP." This rule retires a prior failure where the model wrote "70 POINTS" from a 70 TOTAL_FP: an FP-source number labeled as game points is the same category error FP-VS-POINTS retires for topReason — just on the hand-total side.

CAUSAL CLAIMS — CHOKE-SCOPED: A clause asserting the loss had a cause ("X cost it / killed it / ended it / sank it / did it / lost it") MUST (1) name a specific stat present in SALIENCE or statLine, AND (2) carry magnitude that justifies the verb. No causal-loss claim on an unnamed number, an invented number, or a number too small for the verb. Scope: this rule fires on choke. rare_pull NEVER makes causal-loss claims (the hand is being celebrated, not autopsied) and is not muzzled by this rule — celebrating a held star's own line via topReason stays fully legal. Worked example: WRONG "TWO TURNOVERS COST IT" ("TWO" was never checked against statLine; even if real, two turnovers does not justify "cost it") → RIGHT "SIX TURNOVERS SANK IT" — and only when statLine actually shows 6 turnovers AND that magnitude justifies "sank." This rule retires a prior failure where the model attached a definite causal verb to a fabricated low-magnitude number.

REFERENT — UNIVERSAL: A pronoun in clause 2 (it / he / that / this) requires an explicit noun antecedent in clause 1. No dangling pronouns. The single-clause default (OVERRIDE — STRUCTURE above) already prevents most of this; when a second clause is used, its pronouns must resolve into the first clause. Worked example: WRONG "...TWO TURNOVERS COST IT." ("IT" has no antecedent — the hand, the win, and the night are all absent from clause 1) → RIGHT name the referent: "...TURNOVERS SANK THE WIN." This rule retires a prior failure where a second clause introduced a pronoun with nothing to bind to.
...
export function buildUserPrompt(facts: CommentaryFacts): string {
  ...
  if (facts.totalFp != null) lines.push(`TOTAL_FP: ${facts.totalFp.value} FP`);
  lines.push("");
