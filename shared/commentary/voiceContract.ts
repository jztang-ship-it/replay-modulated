...
ATTRIBUTION — UNIVERSAL: Hand-level totals (TOTAL_FP, NEAR_MISS_GAP_FP, salience aggregates) describe the WHOLE ROSTER, never one player. NEVER write "{N} from {player}" — or any equivalent attribution shape — with a hand-level number. Attribution to a named player is permitted ONLY for player-level fields: anchor.statLine and topReason.value/topReason.label. Carve-out — state it explicitly: topReason.label IS the held star's own line and stays legal — "48 pts (career) from Jordan" is correct because 48 is topReason, not TOTAL_FP. Worked example: WRONG "SEVENTY POINTS FROM GRANGER" (70 = TOTAL_FP, hand-wide, misattributed to one player) → RIGHT either "70 FP ON THE NIGHT" (hand-wide, no player named) or "48 PTS FROM JORDAN" (player-level topReason). Ties to RULE 1: the "N from {player}" shape RULE 1 normalizes ("Twelve points from T-Mac was never enough") is legal ONLY because that N is a player-level number; it does NOT license attributing a hand total to a player. This rule retires a prior failure where the model wrote "SEVENTY POINTS FROM GRANGER" from a 70 TOTAL_FP: the number was real, but pinning a roster total to one name is a category error.

FP-VS-POINTS (EXTENDED) — UNIVERSAL: The FP-VS-POINTS rule above scopes to topReason.value with category="fp". Extend it: ANY number whose SOURCE is an FP field — TOTAL_FP, NEAR_MISS_GAP_FP, actualFp/projectedFp, salience FP contributions — renders as "FP" (or "fantasy points"), NEVER as "points" or "pts." TOTAL_FP is untyped in CommentaryFacts today, so default it to FP. Worked example: WRONG "70 POINTS" when 70 = TOTAL_FP → RIGHT "70 FP." This rule retires a prior failure where the model wrote "70 POINTS" from a 70 TOTAL_FP: an FP-source number labeled as game points is the same category error FP-VS-POINTS retires for topReason — just on the hand-total side.
