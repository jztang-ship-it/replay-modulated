 ...
 * lock.md): segmented into named exported sections so the headline
 * VOICE_CONTRACT (shared/commentary/voiceContract.ts) can inherit the
 * subset that applies cross-surface (register, factual accuracy,
 * trademark, §3 personal life, gold-standard) without dragging in the
 * culture-entry-specific structure and output format. BASKETBALL_VOICE
 * itself is RECOMPOSED as the byte-identical concatenation of the
 * segments — the byte-identity test in __tests__/basketballVoice.byte
 * Identity.test.ts guards that generateCulture.ts produces bit-identical
 * culture entries vs. the pre-refactor SYSTEM string.
 *
...
// ── Private: culture-entry-specific framing (NOT inheritable by headline) ──

const HEADER = `You are a writer for ReplayMod, a fantasy basketball card game. You write player culture entries used as commentary shown after each hand.`;

const TEAM_ERA_SPECIFICITY = `═══ TEAM-ERA SPECIFICITY ═══
...
const FIELD_STRUCTURAL_RULES = `═══ FIELD STRUCTURAL RULES ═══
 ...
 *  rule on top per the Phase 3 lock §B. */
export const BASKETBALL_FACTUAL_ACCURACY = `FACTUAL ACCURACY: Ground every numerical or specific historical claim in the input game data or in well-established public record. Do not invent statistics, draft positions, championship counts, or career milestones. If unsure, omit the specific and use the editorial framing instead.`;

/** Trademark / nominative fair use rule — applies cross-surface verbatim. */
export const BASKETBALL_TRADEMARK = `TRADEMARK USAGE: NBA team names, league name, and player names are used here as editorial reference — nominative fair use. Never write copy that implies an official endorsement, sponsorship, partnership, or affiliation between this product and any team, league, or player. Avoid the words "officially," "endorsed by," "in partnership with," "sponsored by," "brand ambassador" when referring to any team/league/player relationship with this product. Editorial uses of "officially" ("officially arrived as a star," "the era officially ended") are fine — only the affiliation phrasing is restricted.`;

/** §3 personal-life policy — applies cross-surface verbatim. Includes the
 *  CLARIFYING PRINCIPLE ("if a fact wouldn't be discussed during a
 *  basketball broadcast..."). The api/headline.ts denylist enforces a
 *  word-level guard as a backstop. */
export const BASKETBALL_PERSONAL_LIFE = `PERSONAL LIFE: This is a basketball game. Confine criticism to basketball — playing style, draft decisions, contracts, trade dynamics, coaching reputations, on-court controversies. Do not reference players' marriages, romantic partners, domestic incidents, paternity, addiction details, or tabloid storylines.
...
/** Gold-standard + anti-example pairs — register cues, surface-agnostic.
 *  The example lines themselves read as headline-shaped takes (esp.
 *  "63 in the Garden against Bird's Celtics. Casual.") so they inherit
 *  cleanly to the headline surface. */
export const BASKETBALL_GOLD_STANDARD = `═══ GOLD-STANDARD EXAMPLES — MATCH THIS REGISTER ═══
...
// ── Recompose BASKETBALL_VOICE byte-identical to pre-refactor string ───────
// Join with "\n\n" between every section — the original separates each
// section with one blank line in the source template literal. The byte-
// identity guard test pins this against the captured snapshot.

export const BASKETBALL_VOICE = [
  HEADER,
  BASKETBALL_REGISTER,
  ...
  TEAM_ERA_SPECIFICITY,
  FIELD_STRUCTURAL_RULES,
].join("\n\n");
