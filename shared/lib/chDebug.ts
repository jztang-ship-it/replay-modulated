 ...
 *   - Gate: emits ONLY when window.__CH_DEBUG__ === true (opt-in). Off by
 *     default (undefined / any value other than literal true is silent), so
 *     it never logs in production unless explicitly switched on in DevTools.
 *   - SSR-safe: no-op when window is undefined.
 */

export function chDebug(label: string, payload?: Record<string, unknown>): void {
  if (typeof window === "undefined") return;
  if ((window as unknown as { __CH_DEBUG__?: unknown }).__CH_DEBUG__ !== true) return;
