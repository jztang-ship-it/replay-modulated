...
export function GameView({ adapter, challengeCtx, challengeBackCtx, clearChallengeCtx, setChallengeBackCtx, clearChallengeBackCtx, onTakeBoss }: Props) {
  ...
  // Verdict results layout: the gauge bar is gone on this screen, so the bottom
  // grid + the gauge's inner rows collapse the bar's reserved 14px and respend
  // it as even vertical rhythm. Gated on the verdict condition ONLY — NOT
  // hideBar (which also fires during IDLE/DEALING/HOLD/DRAWING and would touch
  // in-game screens + desync the gauge inner height from its outer grid span).
  const verdictLayout = hasVerdict && (gameState === "RESULTS" || gameState === "WIN_CELEBRATION");
  ...
  const voiceLine = useMemo(() => {
    ...
    if (voiceLineRef.current != null && voiceLineKeyRef.current === key) return voiceLineRef.current;
    const line = selectPreVerdictVoice(sportKey, section, handCount * 4 + (roundsUsed ?? 0));
  ...
  const gaugeInert =
    !verdictLayout
    && !gaugeVoice   // a speaking state keeps its 96px voice; the rest show an empty (air) band
