/**
 * useEmotionalReveal.ts — World Cup (ported from basketball, sport-agnostic)
 *
 * Orchestrates the sequential card reveal sequence:
 * - Cards reveal in salary order (cheapest first, anchor last)
 * - Each card flips, FP counts up, badges pop in
 * - Anchor card (most expensive) gets extra drama
 */
