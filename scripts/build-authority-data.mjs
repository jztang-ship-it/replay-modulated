...
const base=join(root,sport,'public','data','seasons');
const sources=readdirSync(base).filter(s=>/^\d{4}$/.test(s)).map(s=>join(base,s));
mkdirSync(join(root,'server-data',sport),{recursive:true});
for(const src of sources){
 const players=JSON.parse(readFileSync(join(src,'players.json'),'utf8'));
 const logs=JSON.parse(readFileSync(join(src,'gamelogs.json'),'utf8'));
...
console.log('Built private basketball authority catalogs');
