  ...
  statDisplay: {
    ...
    winCondition: {
      ...
      thresholds: [
        ...
        { tier: 'JACKPOT',  minFP: 225, multiplier: 0, progressive: true },
      ],
    },
    badges: [
      // ── Scoring
      ...
      // ── Rebounds
      { id: 'BEAST',            icon: '🦍',  label: 'Beast',     est: (s: Record<string,any>) => Number(s.reb) >= 15 },
      ...
      { icon: '🛡️', label: 'Rejection',        fp: 2,  test: (s: Record<string,any>) => Number(s.blk) >= 3 && Number(s.blk) < 5 },
      // ── Efficiency
