...
export function AthleteCardFront(props: {
  ...
  const fadeOpacity = (isRevealing && (visibleFp !== undefined && visibleFp > 0)) ? 0.15 : 1;
  ...
  const candidates = useMemo(() => {
    ...
    console.log('[headshot] card.headshotUrl:', directUrl, 'safeCode:', safeCode);
    if (directUrl) out.push(directUrl);
    if (safeCode) out.push(`/headshots/${safeCode}.png`);
    console.log('[headshot] candidates:', out);
    return out;
  }, [card, safeCode]);
        ...
        <div style={{
          ...
          {badges.map((badge, i) => (
            <div
              key={badge.id}
              style={{
                animation: `badgePop 0.4s cubic-bezier(0.175,0.885,0.32,1.275) ${i * 120}ms both`,
                display: "flex",
            ...
            >
              ...
              <span style={{ fontSize: 9, fontWeight: 700, color: "#FFD700", letterSpacing: 1, marginTop: 2 }}>+{badge.fp}</span>
            </div>
          ))}
        </div>
      )}
